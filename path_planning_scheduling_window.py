...
from planner.utils import view_to_pose_with_target_point, rotation_2_quaternion
import numpy as np
...
def flying_uav(nbv_planner, update_train_event, traj_lock):
    ...
    # planned_traj_csv = setup_csv(nbv_planner.record_path, "planned")
    while time_budget > 0:
        if traj is None:
            time.sleep(1)
            continue
        ...
        # [cyw]: store planned traj
        # pose_nbv  = view_to_pose_with_target_point(nbv)
        # pub_quaternion = rotation_2_quaternion(pose_nbv[:3, :3])
        # pub_position = pose_nbv[:3, -1]
        # pub_pose = [*pub_position, *pub_quaternion]
        # pub_time = time.time()
        # with open(planned_traj_csv, mode='a', newline='') as f:
        #     writer = csv.writer(f)
...
def setup_csv(path, record_type):
    ...
    # elif record_type == "planned":
    #     planned_csv = f"{path}/target_uav_traj.csv"
    #     file_exists = os.path.isfile(planned_csv)
    #     if not file_exists:
    #         with open(planned_csv, mode='w', newline='') as file:
    #             writer = csv.writer(file)
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    global time_budget
    global step
    # time_budget = (args.scheduling_window * args.scheduling_num) + args.initial_training_time
    time_budget = args.flying_time_budget - args.initial_training_time
    ...
    planner_cfg["sampling_num"] = args.sampling_num
    planner_cfg["planning_method"] = args.planning_method
    planner_cfg["radius_start"] = args.radius_start
    planner_cfg["radius_end"] = args.radius_end
    planner_cfg["time_constraint"] = 1
    # planner_cfg["time_constraint"] = args.time_constraint * args.scheduling_window
    planner_cfg["scheduling_window"] = args.scheduling_window
    ...
    global traj
    # traj = nbv_planner.first_plan(args.sampling_method, args.sampling_num) #first traj
    ...
    if args.save_ply_per_time or args.save_ply_each_time:
    ...
    total_time_budget = args.train_time_budget + args.flying_time_budget
    start = time.time()
    while (iteration < final_iteration + 1):
        end = time.time()
        if end - start > total_time_budget:
            scene.save(iteration)
            break

        if time_budget > 0:
            final_iteration += 1
        elif store_last_saving == 1:
            print(f"\n\n run out of time_budget, iteration end at {final_iteration}\n\n")
            store_last_saving = 0
            saving_iterations.append(final_iteration)
        iteration += 1
        ...
        while network_gui.conn != None:
            try:
                ...
                if do_training and ((iteration < final_iteration) or not keep_alive):
                    break
        ...
        if args.save_ply_each_time and (trainig_end_time - start) > args.flying_time_budget and ((trainig_end_time - training_time_saving_start) > 10.0):
            with open(save_3dgs_time_csv, mode='a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([iteration, trainig_end_time - start])
            training_time_saving_start = time.time()
            scene.save(iteration)
        if args.save_ply_per_time  and ((trainig_end_time - training_time_saving_start) > 10.0):
            with open(save_3dgs_time_csv, mode='a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([iteration, trainig_end_time - training_time_saving_start])
            training_time_saving_start = time.time()
            scene.save(iteration)

        if nbv_planner.need_to_update(time_budget, trainig_end_time - trainig_start_time, args.scheduling_window, args.initial_training_time, traj, step):
            try:
                ...
                if planner_type == "ours" or planner_type == "fisher":
                    traj = nbv_planner.plan_path(gaussians, scene, num_views, pipe, background, exit_func=csm.should_exit)
                else:
                    traj = nbv_planner.plan_path()  
                step = 0
                traj_lock.release() 
        ...
        if update_train_event.is_set(): # if the new image is collected, then trigger the event
            if args.save_ply_each_time:
                ...
                with open(save_3dgs_time_csv, mode='a', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow([iteration, tmp_time - start])
                scene.save(iteration)
        ...
        with torch.no_grad():
            ...
            if cur_iter < opt.densify_until_iter:
                ...
                if cur_iter % opt.opacity_reset_interval == 0 or (dataset.white_background and cur_iter == opt.densify_from_iter):
                    if cur_iter >= 3000 and len(scene.train_idxs) < 2: #[cyw]: to prevent from pruning all 3dgs and make the backprop function error
                        time.sleep(1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--planning_method", type=str, default="astar", help="the method planning trajectory for a set of viewports")
    ...
    parser.add_argument("--flying_time_budget", type=float, default=350.0, help="flying_time_budget")
    parser.add_argument("--train_time_budget", type=float, default=350.0, help="train_time_budget")
